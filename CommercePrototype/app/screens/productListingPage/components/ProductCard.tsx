 ...
 * @property {CatalogProduct} product - Product data to render
 ...
 */
export default function ProductCard({ product, onPress, imageStyle, containerStyle }: ProductCardProps) {
  const theme = useTheme();
  const inStock = product.quantityAvailable > 0;
  const categoryLabel = categories.find((c) => c.id === product.categoryId)?.label || "Unknown";
  ...
  return (
    ...
    >
      <Card>
        ...
        <Text style={[styles.category, { color: theme.colors.text, opacity: 0.7 }]}>
          {categoryLabel}
        </Text>
