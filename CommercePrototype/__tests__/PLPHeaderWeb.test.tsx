...
describe("PLPHeaderWebTitle", () => {
  ...
  it("updates the title when the prop changes", () => {
    const { rerender, getByText } = render(<PLPHeaderWebTitle {...defaultProps} />);
  ...
  it("does not render title if empty", () => {
    const { queryByText } = render(<PLPHeaderWebTitle {...defaultProps} title="" />);
    expect(queryByText("Products")).not.toBeInTheDocument();
  });

  it("does not render count if empty", () => {
    const { queryByText } = render(<PLPHeaderWebTitle {...defaultProps} countText="" />);
